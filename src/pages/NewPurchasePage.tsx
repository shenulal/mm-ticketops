...
import { ChevronRight, Lock, CheckCircle, Loader2, CalendarIcon, Plus, AlertTriangle, Paperclip, X } from 'lucide-react';
...
const DELIVERY_CHANNELS = ['e-ticket', 'mobile transfer', 'physical', 'link', 'RFID'] as const;
...
export default function NewPurchasePage() {
  ...
  const [deliveryChannel, setDeliveryChannel] = useState<string>('e-ticket');
    ...
    return (
      <div className="max-w-[800px] mx-auto mt-8 pb-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h2 className="font-heading text-[26px] text-primary text-center mb-1">Purchase Saved Successfully</h2>
          <div className="flex justify-center mb-6">
  ...
  return (
    <div className="max-w-[800px] mx-auto mt-8 pb-12">
      ...
      <h1 className="font-heading text-[28px] mb-1 text-primary">New Purchase Entry</h1>
      ...
      {tab === 'bulk' ? <BulkImportTab mode="PURCHASE" /> : (
        <>
          {/* HEADER */}
          <div className="bg-card rounded-xl shadow-sm p-6 mb-4 space-y-5">
            ...
            {/* Delivery channel + Prepaid */}
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block font-body text-xs font-medium text-foreground mb-1.5">Delivery Channel</label>
                <select value={deliveryChannel} onChange={e => setDeliveryChannel(e.target.value)} className={inputCls('')}>
                  {DELIVERY_CHANNELS.map(ch => <option key={ch} value={ch}>{ch}</option>)}
                </select>
              </div>
              <div>
                <label className="block font-body text-xs font-medium text-foreground mb-1.5">Prepaid / Deposit</label>
                <div className="flex items-center gap-3">
                  <button onClick={() => setPrepaid(!prepaid)}
                    className={cn('w-10 h-5 rounded-full relative transition-colors', prepaid ? 'bg-success' : 'bg-muted')}>
                    <div className={cn('w-4 h-4 rounded-full bg-white absolute top-0.5 transition-all', prepaid ? 'left-5' : 'left-0.5')} />
                  </button>
                  {prepaid && (
                    <input type="number" min={0} value={depositAmount} onChange={e => setDepositAmount(e.target.value)}
                      placeholder="Deposit amount" className="flex-1 h-10 px-3 rounded-lg font-body text-sm border border-border outline-none focus:ring-1 focus:ring-accent bg-card" />
                  )}
                </div>
              </div>
            </div>
            ...
            {/* Attachments */}
            <div>
              <label className="block font-body text-xs font-medium text-foreground mb-1.5">Attachments (invoice / vendor confirmation)</label>
              <div className="flex items-center gap-2 flex-wrap">
                <label className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-border font-body text-xs text-muted-foreground cursor-pointer hover:bg-muted/30">
                ...
                {attachments.map((f, i) => (
                  <span key={i} className="flex items-center gap-1 px-2 py-1 rounded bg-muted font-body text-[11px] text-foreground">
                    {f.name}
                    <button onClick={() => setAttachments(prev => prev.filter((_, idx) => idx !== i))} className="hover:text-destructive"><X size={10} /></button>
                  </span>
                ))}
              </div>
            </div>
                ...
                {lines.map((line, i) => (
                  <motion.div key={line.id}
                    ...
                    <LineItemCard line={line} index={i} matchId={matchId}
                      ...
                      vendorId={vendorId} eventId={activeEvent.id}
              ...
              <div className="mt-4 rounded-lg p-3 bg-amber-50 border border-amber-300">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-body text-xs text-amber-800">
                    {duplicateWarnings.map(w => `Lines ${w.join(' and ')}`).join('; ')} have the same session and category.
                  </p>
                </div>
              </div>
            )}
