...
export default function ClientsPage() {
  ...
  const renderDrawer = (client: Client, onClose: () => void) => {
    const contracts = ctx.contracts.filter(c => c.partyId === client.id);
    return (
      <div>
        ...
        <FieldRow label="Email"><a href={`mailto:${client.email}`} className="text-primary underline">{client.email}</a></FieldRow>
        <FieldRow label="Phone" value={client.phone || '—'} />

        <SectionHeading title="Contracts" count={contracts.length} />
        {contracts.length === 0 ? (
          <p className="text-[12px] text-muted-foreground font-body">No contracts on file.</p>
        ) : contracts.map(c => {
          const ev = ctx.getEvent(c.eventId);
          return (
            <div key={c.id} className="py-2 border-b border-border/50">
              ...
              <div className="text-[11px] text-muted-foreground font-body mt-0.5">{ev?.name ?? c.eventId} · {c.contractType} · {ctx.formatCurrency(c.maxValue)}</div>
            </div>
          );
        })}
