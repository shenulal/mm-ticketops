...
type SaleStatus = 'ALLOCATED' | 'PENDING_APPROVAL' | 'UNALLOCATED' | 'FULFILLED';
...
function buildRows(sales: typeof MOCK_SALES): SaleRow[] {
  return sales.map(s => {
    const m = MOCK_MATCHES.find(m => m.id === s.matchId);
    return {
      id: s.id.toUpperCase(),
      date: '16 Apr',
      match: m ? `${m.code}` : s.matchId,
      client: s.client,
      category: s.category,
      qty: s.qty,
      unitPriceAed: `AED ${Math.round(s.unitPrice * AED_RATE).toLocaleString()}`,
      totalAed: `AED ${Math.round(s.total * AED_RATE).toLocaleString()}`,
      status: s.status,
      oversell: 'oversell' in s ? (s as any).oversell : false,
    };
  });
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-card rounded-2xl shadow-xl max-w-lg w-full mx-4 p-8">
            ...
            <tbody>
              <tr>
                ...
                <td className="px-4 py-3">
                  <span className="font-mono text-sm font-bold" style={{ color: '#DC2626' }}>0 (OVERSOLD)</span>
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div className="flex gap-3">
          <button onClick={onApprove} className="flex-1 py-2.5 rounded-lg font-body text-sm font-bold transition-opacity hover:opacity-90" style={{ backgroundColor: '#D97706', color: 'white' }}>
            Approve Override
          </button>
          <button onClick={onReject} className="flex-1 py-2.5 rounded-lg font-body text-sm font-bold transition-opacity hover:opacity-90" style={{ backgroundColor: '#DC2626', color: 'white' }}>
            Reject Sale
          </button>
        </div>
...
export default function SalesPage() {
  ...
  const rows = buildRows(salesData).filter(r => {
  ...
  const handleApprove = () => {
    setSalesData(prev => prev.map(s => s.id === 's250145' ? { ...s, status: 'ALLOCATED', oversell: false } : s));
    setModalSale(null);
  };
  const handleReject = () => {
    setSalesData(prev => prev.filter(s => s.id !== 's250145'));
    setModalSale(null);
  };

  const oversellRow = rows.find(r => r.oversell);
    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <RoleGuard roles={['super_admin', 'ops_manager', 'sr_operator', 'operator']}>
          <button onClick={() => navigate('/sales/new')} className="px-4 py-2 rounded-lg font-body text-sm font-medium transition-opacity hover:opacity-90" style={{ backgroundColor: '#0B2D5E', color: '#C9A84C' }}>
            New Sale +
          </button>
        </RoleGuard>
      </div>
          ...
          <table className="w-full text-left min-w-[950px]">
            <thead>
              <tr style={{ backgroundColor: '#0B2D5E', height: 44 }}>
                {['Sale ID', 'Date', 'Match', 'Client', 'Category', 'Qty', 'Unit Price', 'Total', 'Status', 'Actions'].map(h => (
            ...
            <tbody>
              {rows.map((r, i) => {
                const st = STATUS_MAP[r.status] || STATUS_MAP['UNALLOCATED'];
                const isOversell = r.oversell;
                return (
                  <>
                    {isOversell && (
                      <tr key={`oversell-${r.id}`}>
                        <td colSpan={10} className="px-4 py-2 font-body text-xs italic" style={{ backgroundColor: '#FEF3C7', color: '#92400E' }}>
                          ⚠ Oversell: Sale {r.id} — {r.client} requested {r.qty} {r.category} units. Current available: 0 over limit. Awaiting manager approval.
                        </td>
                      </tr>
                    )}
                    <tr
                    ...
                    >
                      <td className="px-4 py-3 font-mono text-xs font-bold cursor-pointer hover:underline" style={{ color: '#0B2D5E' }}>{r.id}</td>
      ...
      {modalSale && (
        <ApprovalModal sale={modalSale} onApprove={handleApprove} onReject={handleReject} onCancel={() => setModalSale(null)} />
      )}
    </div>
  );
}
